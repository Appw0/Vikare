...
//        WING_MODELS.put(WingTypes.BOX, Pair.of(BoxWingsModel::new, BoxWingsModel::createLayer));  // debug code
    ...
    public static void registerItemColor(RegisterColorHandlersEvent.Item event) {
        event.register((stack, color) -> color == 0 ? ColorHelper.dyeToDecimal(((WingItem) stack.getItem()).getPrimaryColor()): ColorHelper.dyeToDecimal(((WingItem) stack.getItem()).getSecondaryColor()),
            ...
            if (event.getStage().equals(RenderLevelStageEvent.Stage.AFTER_ENTITIES)) {
                minecraft.level.players().stream().filter(player -> !renderedPlayers.contains(player)).forEach(player ->
                                CuriosApi.getCuriosInventory(player).ifPresent(inventory -> inventory.findFirstCurio(stack -> stack.getItem() instanceof WingItem)
                                        .flatMap(slotResult -> CuriosApi.getCurio(slotResult.stack())
