    ...
    public static class ShinyRecipeVanillaExtension implements ICraftingCategoryExtension {
        ...
        public void setRecipe(IRecipeLayoutBuilder builder, ICraftingGridHelper craftingGridHelper, IFocusGroup focuses) {
            if (focuses.isEmpty()) {
                ...
                craftingGridHelper.createAndSetInputs(builder, VanillaTypes.ITEM_STACK, Arrays.asList(getShinies(), inputs), 0, 0);
                ...
                if (stack.is(ShinyWingsRecipe.SHINE)) {
                    ...
                    WINGS.getEntries().forEach(wings -> {
                        ItemStack shinyItem = new ItemStack(wings.get());
                        shinyItem.enchant(Enchantments.MENDING, 1);

                        ItemStack dullItem = shinyItem.copy();
                        dullItem.getOrCreateTag().putBoolean("Dull", true);

                        inputs.add(shinyItem);
                        outputs.add(dullItem);
                    });

                    craftingGridHelper.createAndSetInputs(builder, VanillaTypes.ITEM_STACK, Arrays.asList(inputs, getShinies()), 0, 0);
                    craftingGridHelper.createAndSetOutputs(builder, VanillaTypes.ITEM_STACK, outputs);
                } else {
                    ...
                    craftingGridHelper.createAndSetInputs(builder, VanillaTypes.ITEM_STACK, Arrays.asList(List.of(stack), getShinies()), 0, 0);
                    craftingGridHelper.createAndSetOutputs(builder, VanillaTypes.ITEM_STACK, List.of(outStack));
                }
            }
        }
